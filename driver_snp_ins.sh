...
intersect_motif_with_tf1_tf2_module_region_by_motif_name_parallel() {
    ...
    echo "python ${merger} -g reference/hg38.fa -i ${motif_dir}/${tf1}_${tf2}_${cell}_${module}_intersect_${tf2}_motif_deleted.bed -o r.${tf1}_${tf2}_${cell}_${module}.bed -r -s -d"
    ...
    fileName="${tf1}_${tf2}_${cell}_${module}_intersect_${tf2}_motif_deleted.bed"
    awk -v dir_prefix="${root_dir}/042523/Input_sequences" -v fileName="${fileName}" 'BEGIN{OFS="\t"} {n=split($6, a, ","); for(i=1; i<=n; i++) {print $1, $2, $3, $4, toupper($5), toupper(a[i]) > dir_prefix"/S"(i%5+1)"/"fileName } }' r.${tf1}_${tf2}_${cell}_${module}.bed
    rm r.${tf1}_${tf2}_${cell}_${module}.bed
...
prepare_sequences_for_ref_variant() {
    ...
    do
        ...
        bed_file=${root_dir}/042523/Input_sequences/S${i}/${tf1}_${tf2}_${cell}_${module}_intersect_${tf2}_motif_deleted.bed
        vcf_file=${root_dir}/042523/Input_sequences/S${i}/${tf1}_${tf2}_${cell}_${module}_intersect_${tf2}_motif_deleted.vcf
...
tf_model_prediction_parallel_work_multi_nodes() {
    ...
    do
        ...
        sbatch --wait batch_prepare_sequences_for_ref_variant.sh ${tf1} ${tf2} ${cell} ${module} &
...
tf_model_prediction_parallel() {
    ...
    cat files | awk -F "/" '{print "042523/Input_sequences/"$(NF-1)"/"$NF}' | parallel -k -j 20 wc -l | awk '{print $1}' > rs
